...
/**
 * Tests TweetCentroid. Run from the command line with: <p/>
 * java weka.filters.unsupervised.attribute.TweetCentroidTest
 * <p> 
 ...
 */
public class TweetCentroidTest extends AbstractFilterTest {

    public TweetCentroidTest(String name) {
        super(name);
    }

    /** Creates a default TweetCentroid filter */
	...
	if ((new File(".." + File.separator + "AffectiveTweets" + File.separator + "build_package.xml")).exists()) {
	    ...
	    f = new TweetCentroid(); // Hope that the package is installed.
	}
	return f;
    }

    /**
     * TweetCentroid is not suitable for use in a FilteredClassifier, so this just creates a dummy
    ...
    public static Test suite() {
        return new TestSuite(TweetCentroidTest.class);
    }
